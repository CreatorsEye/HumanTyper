...
// Include the new isolated engine
#include "TypingEngine.h"

// ------------------------------------------------------------------
// Global State & Settings (v1.4)
// ------------------------------------------------------------------
static int spdIdx = 2;
static int mstIdx = 0;
static int totalWords = 15;

// Increased buffer size to safely hold large Chinese/Arabic UTF-8 bytes
static char inputBuf[16384] = "This is simple auto writer tool used for good propose.\nFree to use.\nGreat to achieve your goals.";

// Definition of external variables used in TypingEngine
std::atomic<bool> isTyping(false);
std::atomic<bool> isPaused(false);
std::atomic<bool> stopTyping(false);

std::string engineLog = "[12:00:00] CE Commander Engine V1.4 \n> Universal World Fonts Loaded (Arabic/CJK/RTL).\n> GUI and Core Logic split.\n> Clipboard & Notepad Ready.\n";
std::mutex logMutex;
...
// Safe Font File Checker to prevent ImGui fatal aborts
inline bool FileExistsSafe(const std::string& name) {
...
int main() {
    ...
    HANDLE hMutex = CreateMutexA(NULL, FALSE, "CE_AutoTyper_SingleInstance_Mutex_v1_4");
    ...
    GLFWwindow* window = glfwCreateWindow(800, 640, "CE Commander Engine v1.4", NULL, NULL);
    if (!window) return 1;
    ...
    // --------------------------------------------------------------
    // CRASH-PROOF UNIVERSAL FONT LOADER
    // --------------------------------------------------------------
    ImFontConfig fontCfg;
    fontCfg.OversampleH = 1; 
    fontCfg.OversampleV = 1;
    
    static const ImWchar universal_ranges[] = {
    ...
    if (FileExistsSafe("C:\\Windows\\Fonts\\arial.ttf")) {
        io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\arial.ttf", 18.0f, &fontCfg, universal_ranges);
        baseLoaded = true;
    } else if (FileExistsSafe("C:\\Windows\\Fonts\\tahoma.ttf")) {
        io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\tahoma.ttf", 18.0f, &fontCfg, universal_ranges);
        baseLoaded = true;
    }

    if (!baseLoaded) { io.Fonts->AddFontDefault(); }
    ...
    if (FileExistsSafe("C:\\Windows\\Fonts\\msyh.ttc")) {
        io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\msyh.ttc", 18.0f, &mergeCfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
    } else if (FileExistsSafe("C:\\Windows\\Fonts\\simhei.ttf")) {
        io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\simhei.ttf", 18.0f, &mergeCfg, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
    }
    // --------------------------------------------------------------
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        ImGui::TextColored(ImVec4(0.6f, 0.7f, 0.9f, 1.0f), "CE Commander Engine v1.4");
        ...
        // Formatted and LEFT-Aligned Hotkey Label (Fixed RTL/Right-Alignment visual bug)
        ...
        if (ImGui::Button("Clipboard", ImVec2(100, 30))) {
            ...
            if (clipboardText && strlen(clipboardText) > 0) {
                snprintf(inputBuf, sizeof(inputBuf), "%s", clipboardText);
