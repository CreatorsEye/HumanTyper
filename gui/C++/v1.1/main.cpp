...
std::string engineLog = "[12:00:00] CE Commander Engine V1.1 \n> GUI upgraded to Borderless.\n> Global Hotkeys Activated.\n";
std::mutex logMutex;

// Hardcoded Global Hotkeys (Win32 Virtual Keys)
#ifdef _WIN32
static int hkStart = VK_F7; // Updated from F5 to F7
...
int main() {
    ...
    // v1.1 BORDERLESS WINDOW
    glfwWindowHint(GLFW_DECORATED, GLFW_FALSE);
    glfwWindowHint(GLFW_VISIBLE, GLFW_FALSE);
    
    GLFWwindow* window = glfwCreateWindow(800, 600, "CE Commander Engine v1.1", NULL, NULL);
    if (!window) return 1;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // --------------------------------------------------------------
        // GLOBAL HOTKEY LISTENER (Background Polling)
        // --------------------------------------------------------------
        ...
        // --------------------------------------------------------------
        // CUSTOM DRAG-AND-DROP TOP BAR WITH EXIT BUTTON
        // --------------------------------------------------------------
        ...
        ImGui::Text("CE Commander Engine v1.1");
        ...
        // --------------------------------------------------------------
        // MAIN UI BODY (Shifted down below TitleBar)
        // --------------------------------------------------------------
        ...
        // HARDCODED HOTKEY LABEL DISPLAYED DIRECTLY BELOW TEXT INPUT
        ImGui::SetCursorPos(ImVec2(20, 195));
        // Updated text label from F5 to F7
        ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.2f, 1.0f), "Hotkeys -> Start: F7 | Pause: F8 | Resume: F9 | Stop: F10 | Status: F11");
