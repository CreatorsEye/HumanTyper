...
namespace CeAutoTyper
{
    public class MainForm : Form
    {
        ...
        private ComboBox cmbMistakes;          // NEW
        ...
        private Random rand = new Random();    // for random mistake count
            ...
            try
            {
                using (Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("CeAutoTyper.ce.ico"))
                {
                    if (stream != null)
                    {
                        ...
                        trayIcon.Text = "CE Auto Typer v1.6";
        ...
        // ----- Helper: get mistake count from dropdown -----
        ...
        private async void BtnStart_Click(object? sender, EventArgs e)
        {
            ...
            if (!engine.StopTyping)
            {
                ...
                int mistakeCount = GetMistakeCount();   // NEW
                engine.SetSpeed(speedIndex);
                engine.SetMistakeCount(mistakeCount);   // NEW
    ...
    // ==========================================
    // TYPING ENGINE WITH MISTAKES (FUNCTIONAL)
    // ==========================================
    public class TypingEngine
    {
        ...
        // Create a typo (double letter, missing letter, or swap)
        ...
        public async Task TypeTextAsync(string text)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                // Decide if we should make a mistake on this word
                ...
                {
                    // Find the whole word boundaries
                    int start = i, end = i;
                    ...
                    if (word.Length > 2)
                    {
                        ...
                        if (!stopRequested)
                        {
                            // Pause, then backspace all characters of the typo
                            ...
                            mistakesMade++;
                            i = end;   // skip the rest of the word
                            continue;
                        }
                    }
                }
